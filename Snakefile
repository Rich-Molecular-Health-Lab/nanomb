...
ROUTE      = load_routing_for_dataset(DATASET)   
...
# ---- run mode helpers ----
_MODE = (config.get("mode", "full") or "full").lower()
def do_known():     return _MODE in ("full", "knowns_only")
def do_unknowns():  return bool(config.get("unknowns",{}).get("enable", False)) and _MODE in ("full","unknowns_only")
...
def _unk_table_input(wc):
    return (UNKNOWN_TABLE_MERGED 
            if enabled_unknowns()
            else os.path.join(OUT, "otu", ".empty_unknown_table.tsv"))
...
def filtered_stems_from_manifest(wc):
    mf = checkpoints.fastcat_filter.get().output.files_manifest
    stems = set()
    with open(mf) as fh:
        for line in fh:
            fn = line.strip()
...
def paf_targets(wc):
    stems = filtered_stems_from_manifest(wc)
...
def _known_table_input(wc):
    return (os.path.join(OUT, "otu/otu_table_merged.tsv")
            if do_known()
            else os.path.join(OUT, "otu", ".empty_known_table.tsv"))

def _sync_tree_input(wc):
     if do_unknowns():
         return os.path.join(OUT, "otu/otu_tree.with_unknowns.nwk")
     return os.path.join(OUT, "otu/otu_tree.treefile")

def _sync_unknown_fasta(wc):
     if do_unknowns():
         return UNKNOWN_CENT_FILT
     return os.path.join(OUT, "otu", ".empty_unknowns.fasta")

if not enabled_unknowns():
    rule _empty_unknown_table:
        output: os.path.join(OUT, "otu", ".empty_unknown_table.tsv")
        shell: "mkdir -p $(dirname {output}); printf 'OTU\n' > {output}"

if not do_known():
    rule _empty_known_table:
        output: os.path.join(OUT, "otu", ".empty_known_table.tsv")
        shell: "mkdir -p $(dirname {output}); printf 'OTU\n' > {output}"
...
checkpoint fastcat_filter:
    input:
        fastqs = all_trimmed_fastqs,
        deps   = [ancient(os.path.join(raw_dir_for_run(r), "demux_trim.done")) for r in RUNS]
    output:
...
rule nanoplot_qc:
    input:
        done  = lambda wc: checkpoints.fastcat_filter.get().output.done
    output:
    ...
    shell: r"""
      ...
      fq_list=({params.filt_dir}/*.fastq)
      if (( ${{#fq_list[@]}} == 0 )); then
        # Nothing to plot; create an empty file so NanoPlot doesn't choke
        : > {output.outdir}/all.fastq
      else
        cat "${{fq_list[@]}}" > {output.outdir}/all.fastq
      fi
...
# ---------------- Clustering & Mapping (Pooled) -------------

rule pooled_concat_reads:
    input:
        reads = lambda wc: checkpoints.fastcat_filter.get().output.done
    output:
        pooled = POOLED_FQ
...
# ---------------- Clustering & Mapping (Unpooled) -------------

rule isonclust3:
    input: lambda wc: checkpoints.fastcat_filter.get().output.done
    output: 
...
rule otu_table_per_sample:
    input:
        ...
        deps_done = lambda wc: checkpoints.fastcat_filter.get().output.done
    output:
...
rule iqtree3_model_extract:
    ...
    run:
        ...
        s = s.replace("+FQ", "").replace("+F", "") 
        s = re.sub(r"\+R\d+", "", s)               
        s = re.sub(r"\+G\d+", "+G", s)             
        s = re.sub(r"\+FO?", "", s)                
    
        base = re.split(r"\+", s)[0]
        gtr_like = {"SYM", "TIM", "TVM", "TRN", "TPM", "K3P", "TAMURA", "GTR2"}
        if base in gtr_like:
            base = "GTR"
    
        allowed = {"JC", "K80", "HKY", "TN93", "GTR"}
        if base not in allowed:
            base = "GTR"
    
        flags = []
        if "+I" in s: flags.append("+I")
        if "+G" in s: flags.append("+G")
        if not flags:  
            flags = ["+G"]
    
        normalized = base + "".join(flags)
...
rule itgdb_map_reads:
    input:
        filt_done = lambda wc: checkpoints.fastcat_filter.get().output.done,
        idx   = ancient(os.path.join(OUT, "itgdb/index", "ITGDB_16S.mmi"))
    output:
...
if enabled_unknowns():
    ...
    rule align_unknowns_to_backbone:
        output: q_aln = os.path.join(OUT, "otu/unknowns_aligned.fasta")
        resources:
            mem_mb    = Rq("align_unknowns_to_backbone", "mem_mb"),
            runtime   = Rq("align_unknowns_to_backbone", "runtime"),
            partition = Rq("align_unknowns_to_backbone", "partition"),
            account   = Rq("align_unknowns_to_backbone", "account"),
            extra     = R("align_unknowns_to_backbone", "extra")
        shell: "mkdir -p $(dirname {output.q_aln}); : > {output.q_aln}"

    rule epa_place_unknowns:
        output: jplace = os.path.join(OUT, "otu/unknowns.placements.jplace")
        resources:
            mem_mb    = Rq("epa_place_unknowns", "mem_mb"),
            runtime   = Rq("epa_place_unknowns", "runtime"),
            partition = Rq("epa_place_unknowns", "partition"),
            account   = Rq("epa_place_unknowns", "account"),
            extra     = R( "epa_place_unknowns", "extra")
        shell: "mkdir -p $(dirname {output.jplace}); echo '{{\"tree\": \"\", \"placements\": []}}' > {output.jplace}"
    ...
    rule graft_unknowns_on_tree:
        ...
        resources:
            ...
            account   = Rq("graft_unknowns_on_tree", "account"),
            extra     = R( "graft_unknowns_on_tree", "extra")
        shell: "cp {input.tree} {output.tree_grafted}"

# ---- helpers that choose inputs for sync ----
            
if not enabled_unknowns():
    rule _empty_unknowns_fasta:
        output: os.path.join(OUT, "otu", ".empty_unknowns.fasta")
        shell: "mkdir -p $(dirname {output}); : > {output}"            

rule merge_known_unknown:
    input:
        known = _known_table_input,
        unk   = _unk_table_input
    output:
    ...
    run:
        ...
        known, unk, out = input.known, input.unk, output.merged
        
        try:
            if os.path.exists(unk) and os.path.exists(known) and os.path.samefile(known, unk):
        ...
        def load_tsv(path):
            ...
            with open(path, newline="") as fh:
                rows.extend(csv.reader(fh, delimiter="\t"))
            return rows
        ...
        def build_map(rows, header_samples):
            ...
            for r in rows:
                ...
                m[otu] = {s: (int(float(v)) if v and v != "NA" else 0)
                          for s, v in zip(header_samples, vals)}
            return m

        km = build_map(k_rows, k_samples)
        um = build_map(u_rows, u_samples)

        otus = sorted(set(km) | set(um))
        with open(out, "w", newline="") as fh:
            w = csv.writer(fh, delimiter="\t")
            w.writerow(["OTU"] + all_samples)
            for o in otus:
                w.writerow([o] + [km.get(o, {}).get(s, 0) + um.get(o, {}).get(s, 0)
...
rule target_knowns_only:
    input:
        rules.otu_taxonomy_best.output.tax,
        rules.otu_table_per_sample.output.merged,
        rules.iqtree3_tree.output.tree

rule target_unknowns_only:
    input:
        rules.unknown_tbl_per_samp.output.merged if enabled_unknowns() else os.path.join(OUT, "otu", ".empty_unknown_table.tsv"),
        rules.graft_unknowns_on_tree.output.tree_grafted if enabled_unknowns() else rules.iqtree3_tree.output.tree

rule target_merge_and_exports:
    input:
        OTU_TABLE_ALL,
        OTU_TABLE_FINAL,
        os.path.join(OUT, "otu/otu_table_final.synced.tsv"),
...
# -------- mode-aware default targets --------

if _MODE == "knowns_only":
    _mode_targets = [
        rules.target_knowns_only.output if hasattr(rules, "target_knowns_only") else [],
        rules.target_merge_and_exports.output if hasattr(rules, "target_merge_and_exports") else [],
    ]
elif _MODE == "unknowns_only":
    _mode_targets = [
        rules.target_unknowns_only.output if hasattr(rules, "target_unknowns_only") else [],
        rules.target_merge_and_exports.output if hasattr(rules, "target_merge_and_exports") else [],
    ]
else:
    _mode_targets = _final_targets
