...
```{bash}
srun --job-name=smoke --mem=16gb --nodes=1 --ntasks-per-node=4 --time=3:00:00 --pty bash -l
```
...
apptainer pull trimal.sif docker://staphb/trimal:latest
...
```{bash}
docker buildx build --platform linux/amd64 -t aliciamrich/biopython-sync:1.0 --push .
```

```{bash}
apptainer exec "biopython.sif" python3 - <<'PY'
...
```{bash}
/mnt/nrdstor/richlab/shared/datasets/16s/loris/culi_archive
```
